...
# Because signup dates are sampled with randint(0, 29), the actual last signup date is 2025-03-01.
...
# Gross Margin: Deezer FY2024 adjusted GM was 25.9%, simplified conservatively to 25%.
GROSS_MARGIN  = 0.25

# Monthly Churn Rate: 2% is a reasonable music streaming industry benchmark.
MONTHLY_CHURN = 0.02
...
CONV_RATE_MAX  = 0.06   # Treatment upper bound
...
CTR_TREATMENT_RP  = 0.14   # Lower RP CTR due to traffic shifting toward recommendations
...
for i in range(NUM_USERS):
    ...
    signup = COHORT_START + timedelta(days=int(np.random.randint(0, signup_range_days)))
...
for user in users:
    ...
    for day_offset in range(obs_days + 1):
        ...
        if user_group == 'control':
            ...
            p_rec     = 0.55 if has_event else 0.45
...
# Trial Conversion target: Control ~3-4%, Treatment ~4-5%
